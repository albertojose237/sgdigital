...
export function AboutSection() {
  ...
  return (
    <section id="sobre" className="py-8 sm:py-10 md:py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-center">
          ...
          <div ref={imageRef} className="relative h-56 sm:h-72 md:h-125 lg:h-150 order-2 lg:order-1 rounded-2xl overflow-hidden group fade-in-observer slide-left-observer">
            ...
            <div className="absolute bottom-0 left-0 right-0 h-20 sm:h-24 md:h-32 lg:h-40 bg-linear-to-t from-black/80 via-black/40 to-transparent flex items-end p-3 sm:p-4 md:p-6 lg:p-8">
              <h3 className="text-base sm:text-lg md:text-2xl lg:text-3xl font-bold text-white leading-tight">INOVAÇÃO E CRESCIMENTO PARA O SEU NEGÓCIO</h3>
            </div>
          </div>
          ...
          <div ref={cardRef} className="lg:-ml-16 relative z-10 order-1 lg:order-2 fade-in-observer slide-right-observer">
            <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 lg:p-12">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-700 leading-relaxed mb-3 sm:mb-4 md:mb-6">
                ...
                excelência, construímos o futuro digital do seu negócio.
              </p>
                ...
                {values.map((value, index) => (
                  <div key={index} className="text-center">
                    <div className="flex justify-center mb-2 sm:mb-3">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-orange-500 rounded-full flex items-center justify-center text-white text-lg sm:text-xl md:text-2xl">
                        {value.icon}
                      </div>
                    </div>
                    <h3 className="text-xs sm:text-sm md:text-base font-bold text-blue-600 mb-1 sm:mb-2">{value.title}</h3>
                    <p className="text-xs text-gray-700 leading-relaxed">{value.description}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-end pt-3 sm:pt-4 border-t border-gray-300">
                <a
                  href="#contato"
                  className="text-xs sm:text-sm md:text-base text-blue-600 font-semibold hover:text-orange-500 transition-colors inline-flex items-center gap-2 cursor-pointer"
                >
